...
<div class="flex h-screen">
    ...
    @php
    $allowedEmails = ['penerimaan@gudang.com', 'manager@gudang.com'];
...
@endphp
    <div class="flex-1 flex flex-col overflow-y-auto">
        ...
        <header class="flex items-center justify-between bg-white shadow px-6 py-4">
            <h1 class="text-lg font-semibold">Penerimaan Barang</h1>
            <div class="relative">
        ...
        <main class="p-6 w-full">
            <div class="max-w-6xl mx-auto">
                ...
                <div class="flex justify-between items-center mb-4">
                    <!-- Form Pencarian -->
                    <form method="GET" action="{{ route('penerimaan_barang.index') }}" class="relative w-full max-w-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2">Jumlah(unit)</th>
                        ...
                        <tbody>
                            @foreach ($penerimaan as $index => $item)
                                <tr class="border-t">
                                    ...
                                    <td class="px-4 py-2">{{ $item->barang->jenis_barang }}</td>
                                    <td class="px-4 py-2">{{ $item->supplier->nama_supplier ?? '-' }}</td>
                                    <td class="px-4 py-2">{{ $item->jumlah_diterima }}</td>
                                    ...
                                    <td class="px-4 py-2 flex gap-2">
                                        <div class="flex gap-2">
                                             @if($isAllowed)
    {{-- Tombol Edit --}}
    <a href="{{ route('penerimaan_barang.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800">
                                        ...
                                        <div id="modal-{{ $item->id }}" class="fixed z-50 inset-0 bg-black bg-opacity-30 hidden justify-center items-center">
                                            <div class="bg-white p-6 rounded shadow-md max-w-md w-full">
                                                ...
                                                <div class="flex justify-end mt-4 gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('penerimaan_barang.destroy', $item->id) }}">
                                                        @csrf
                                                        @method('DELETE')
