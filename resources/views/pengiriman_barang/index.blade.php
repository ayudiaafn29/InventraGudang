...
<body class="bg-gray-100 font-poppins">
 ...
 @php
    $allowedEmails = ['pengantar@gudang.com','manager@gudang.com'];
...
@endphp
        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
            ...
            <header class="flex items-center justify-between bg-white shadow px-6 py-4">
                <h1 class="text-lg font-semibold">Pengiriman Barang</h1>
                <div class="relative">
            ...
            <main class="p-6 w-full">
                <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <!-- Form Pencarian -->
                        <form method="GET" action="{{ route('pengiriman_barang.index') }}" class="relative w-full max-w-sm">
...
@if($isAllowed)
                        <a href="{{ route('pengiriman_barang.create') }}"
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-4 py-2">Nama Barang Dikirim</th>
                            ...
                            <tbody>
                                @foreach ($pengiriman as $index => $item)
                                    <tr class="border-t">
                                        ...
                                        <td class="px-4 py-2">
                                            {{ $item->pengemasan->permintaan->penyimpanan->penerimaan->barang->kode_barang ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->pengemasan->permintaan->penyimpanan->penerimaan->barang->nama_barang ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->nama_penerima ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->pengemasan->tujuan_pengiriman ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->tanggal_dikirim ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                          @if ($item->status_pengiriman === 'sedang dikirim')
    <span class="text-yellow-600 font-semibold">Sedang Dikirim</span>
@elseif ($item->status_pengiriman === 'selesai dikirim')
    <span class="text-green-600 font-semibold">Selesai Dikirim</span>
@else
<span class="text-yellow-600 font-semibold">Sedang Dikirim</span>
...
@endif
                                        ...
                                        <td class="px-4 py-2 flex items-center space-x-2">
                                            <!-- Edit -->
                                            @if($isAllowed)
                                            <a href="{{ route('pengiriman_barang.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800">
                                            ...
                                            <div id="modal-{{ $item->id }}" class="fixed z-50 inset-0 bg-black bg-opacity-30 hidden justify-center items-center">
                                                <div class="bg-white p-6 rounded shadow-md max-w-md w-full">
                                                    ...
                                                    <div class="flex justify-end mt-4 gap-2">
                                                        ...
                                                        <form method="POST" action="{{ route('pengiriman_barang.destroy', $item->id) }}">
                                                            @csrf
                                                            @method('DELETE')
...
@endif
    ...
    searchForm.addEventListener('submit', function (e) {
        ...
        // Jika ingin langsung submit ke server (tanpa cek manual)
        searchForm.submit();

        // Atau jika pakai AJAX, kirim data di sini
    });
});
