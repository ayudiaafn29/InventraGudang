...
<body class="bg-gray-100 font-poppins">
 ...
 @php
    $allowedEmails = ['pengemasan@gudang.com', 'manager@gudang.com'];
...
@endphp
        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
            ...
            <header class="flex items-center justify-between bg-white shadow px-6 py-4">
                <h1 class="text-lg font-semibold">Pengemasan Barang</h1>
                <div class="relative">
            ...
            <main class="p-6 w-full">
                <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <!-- Form Pencarian -->
                        <form method="GET" action="{{ route('pengemasan_barang.index') }}" class="relative w-full max-w-sm">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-4 py-2">Nama Barang Dikemas</th>
                                    <th class="px-4 py-2">Jumlah Barang</th>
                                    <th class="px-4 py-2">Tujuan Pengiriman</th>
                                    <th class="px-4 py-2">Tanggal Pengemasan</th>
                                    <th class="px-4 py-2">Status</th>
                                    <th class="px-4 py-2">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($pengemasan as $index => $item)
                                    <tr class="border-t">
                                        <td class="px-4 py-2">{{ $index + 1 }}</td>
                                        <td class="px-4 py-2">
                                            {{ $item->permintaan->penyimpanan->penerimaan->barang->kode_barang ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->permintaan->penyimpanan->penerimaan->barang->nama_barang ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->permintaan->jumlah_barang_diminta ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->tujuan_pengiriman ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->tanggal_pengemasan ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2">
                                          @if ($item->status_pengemasan === 'sedang dikemas')
    <span class="text-yellow-600 font-semibold">Sedang Dikemas</span>
@elseif ($item->status_pengemasan === 'siap dikirim')
    <span class="text-green-600 font-semibold">Siap Dikirim</span>
@else
<span class="text-yellow-600 font-semibold">Sedang Dikemas</span>
...
@endif
                                        ...
                                        <td class="px-4 py-2 flex items-center space-x-2">
                                            <!-- Edit -->
                                            @if($isAllowed)
                                            <a href="{{ route('pengemasan_barang.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800">
...
<image id="image0_284_1087" width="90" height="90" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
                                            ...
                                            <!-- Hapus -->
                                            <button onclick="openModal({{ $item->id }})" class="text-red-600 hover:text-red-800">
                                               <svg width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                            ...
                                            <div id="modal-{{ $item->id }}" class="fixed z-50 inset-0 bg-black bg-opacity-30 hidden justify-center items-center">
                                                <div class="bg-white p-6 rounded shadow-md max-w-md w-full">
                                                    ...
                                                    <div class="flex justify-end mt-4 gap-2">
                                                        ...
                                                        <form method="POST" action="{{ route('pengemasan_barang.destroy', $item->id) }}">
                                                            @csrf
                                                            @method('DELETE')
